...
| **G1.2** *(closed v2.5, 2026-05-07)* | ~~HMCTS IdP support for OAuth 2.0 client_credentials grant for service principals~~ — **no longer required at MVP.** At MVP all NJI runtime calls are user-initiated and inter-service auth is via JWT propagation; HMCTS IdP only needs to handle human OIDC `authorization_code` flow. The mTLS fallback narrative is also retired. **If non-user-initiated flows are introduced post-MVP**, the service-identity question reopens — see G7. | Closed; superseded by G7 for any post-MVP service-identity needs |
...
| **G3.6** *(new 2026-05-06)* | **UK West DR scope decision (cold-standby).** NJI's HA topology is same-Azure-region multi-AZ in UK South; UK West is reserved for disaster recovery (cold-standby, manual failover playbook). Whether DR is in **MVP scope** is a programme decision. | If DR is in scope: PostgreSQL geo-redundant backup with UK West restore target; AKS cluster Helm definitions (`values-dr-uk-west.yaml`) held but cluster *not* pre-provisioned (cost saving); Azure Front Door / Traffic Manager DNS failover playbook documented; RTO/RPO targets agreed with HMCTS. If DR is post-MVP: programme accepts the residual full-region-UK-South-unavailability risk for the MVP and pilot waves; revisits before broad GA. **Default position: post-MVP** — confirm with programme. |
...
| **G4.5** | Spectral OpenAPI lint ruleset codified for the API-as-Product standards (consistent error envelope, RFC 7807 references, versioning prefix) | Phase 0 deliverable as part of scaffolding |
...
| **G6.2** *(refined 2026-05-06)* | **Single-DB blast radius.** A PostgreSQL outage affects every NJI service simultaneously (vs per-service DBs where one DB outage affects only one service). | Mitigation refined: PostgreSQL Flexible Server **zone-redundant HA** within UK South — primary + standby in different AZs, synchronous replication, automatic failover (<60 s typical). Single-AZ failure inside UK South is tolerated transparently. Point-in-time restore retained. **Full-region UK South loss is the residual risk**, addressed by UK West DR if approved (G3.6); without DR, the residual risk is accepted for MVP and pilot waves. Acceptable trade for the operational simplicity of one DB at NJI's bounded scale. |
...
| ID | Gap | Notes |
|---|---|---|
| **G7.1** *(new v2.5, 2026-05-07)* | **Service-identity mechanism for non-user-initiated flows is undecided.** At MVP every NJI runtime call is user-initiated, so JWT propagation suffices and no NJI service principals exist. Any of the following would re-open the service-auth question: (a) DA&I post-MVP integration with NJI MI Feed (DA&I might be a service consumer rather than a human user); (b) async messaging / event-bus patterns; (c) scheduled background jobs (e.g. nightly reconciliation triggers, MI Feed pre-aggregation); (d) the Phase 0 ETL evolving from operator-initiated to fully automated (per G4.7); (e) cross-system integrations beyond MI consumers (e.g. future programmes auto-pulling MI). | **Open question for post-MVP.** Options to consider when the question is reopened: <br/>(A) **NJI-internal service-auth issuer** — a small Spring Authorization Server-based service (similar to today's mock-auth, but production-hardened) that issues `client_credentials` tokens for NJI service principals only. JWTFilter configured for multi-issuer (HMCTS IdP for humans + nji-service-auth for services). <br/>(B) **Azure Workload Identity / Managed Identity** — cloud-native; no shared secrets; tokens issued by Entra ID and validated via its JWKS. <br/>(C) **mTLS** — per-service certs at TLS layer; operationally heavy but cryptographically strong. <br/>(D) **HMCTS IdP `client_credentials`** — only if HMCTS IdP supports it (the original v1.x assumption — never verified). <br/>Default: defer until a concrete need arrives; revisit before broad GA. |
