...
| **A24** | The 11-service decomposition (revised v2.2, 2026-05-07 — `ram-configuration` dropped in favour of Spring profiles + Key Vault and a shared `configuration_values` infrastructure table) is programme-approved and binding | Load-bearing | Per PRD |
...
| **A29** | Schema evolution: Flyway, per the HMCTS Crime SpringBoot template (`spring-boot-starter-flyway` + `flyway-core` + `flyway-database-postgresql`). Flyway owns RAM Pathfinder's own DDL — table creation, column add/drop, grants. **Not** APEX-to-RAM Pathfinder data movement; the Phase 0 ETL (see [`../architecture.md`](../architecture.md)) loads RAM Pathfinder data via the Reference Data and Authorisation APIs. | Confirmed via template review (2026-05-06) |
...
| **A32** *(revised 2026-05-06)* | Phase 0 Reference Data + Users/Roles migration is an ETL: read APEX SQL exports → transform to RAM Pathfinder shape → load via RAM Pathfinder Reference Data API and Authorisation API. Operationally feasible with named-owner sign-off. | Load-bearing | Phase 0 deliverable; reconciliation per D3 + Risk #13. Tool at `ram-architecture/migration/`. |
| **A33** *(revised 2026-05-06)* | The Authoritative Table Ownership Mapping (39 RAM Pathfinder tables in [`./data-tables.md`](./data-tables.md)) is RAM Pathfinder's design, fixed by RAM Pathfinder; not revalidated against APEX shape. What is revalidated in Phase 0 is the migration tool's APEX-side input mapping: when the dump arrives, verify the tool covers every APEX field RAM Pathfinder needs and every APEX value lands in a known RAM Pathfinder vocabulary row (new values added via the Reference Data API; no schema change). An unmapped APEX structure raises an architectural PR. | Load-bearing for migration correctness | Phase 0 deliverable (G4.6); architecture changes only if an unmapped APEX structure surfaces. |
