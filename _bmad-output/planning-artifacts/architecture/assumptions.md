...
| **A2** *(retired v2.5, 2026-05-07)* | ~~HMCTS IdP supports OAuth 2.0 client_credentials grant for service principals~~ — **no longer required at MVP.** Superseded by A35 (all MVP runtime calls are user-initiated; JWT propagation is the only inter-service auth). G1.2 closes; G7 captures the post-MVP open question. | Retired |
...
| **A26** *(narrowed v2.5, 2026-05-07)* | Mock auth implementation (Spring Authorization Server-based) provides OIDC `authorization_code` flow contract parity with HMCTS IdP for the human-user authN paths NJI services use. *(The earlier `client_credentials` clause is retired — at MVP NJI does not use service principals.)* | Load-bearing | Phase 0 deliverable; integration tests verify human-flow contract parity |
...
| **A35** *(new v2.5, 2026-05-07)* | **All MVP runtime requests into NJI services are user-initiated.** This is the operating assumption that justifies the MVP auth simplification: HMCTS IdP handles only human OIDC `authorization_code` flow; NJI does not issue or validate service-principal tokens; inter-service auth is by JWT propagation (forwarding the user's `Authorization: Bearer …` header). Any non-user-initiated runtime flow (background jobs, scheduled aggregations, async messaging, DA&I service-consumer calls) is **out of scope at MVP**. Phase 0 production data migration runs operator-initiated (named programme operator authenticates and runs the ETL with their own JWT). Dev/CI environment seeding is via one-off scripts that don't traverse the runtime API. | Load-bearing | If a non-user-initiated flow is introduced post-MVP the assumption breaks and the service-identity question reopens — see [`./gaps.md` G7](./gaps.md). |
