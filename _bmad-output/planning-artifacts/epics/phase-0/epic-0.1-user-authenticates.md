...
- `nji-ui` repo scaffolded (React + TypeScript + Vite + Vitest + Playwright; per AR42–AR43)
- `HmctsIdpProvider`, `ProtectedRoute`, `useAuth`, HTTP client with RFC 9457 error handling (per AR44)
- GOV.UK Design System base + HMCTS/NJI extensions
- Home shell with role-scoped nav + Region/Area selector (FR55)
- Shared `configuration_values` Flyway baseline established by `nji-architecture` (FR8)
...
## Story 0.1.1: Scaffold `nji-authorisation` service from HMCTS Crime SpringBoot template

As a **platform engineer**,
I want to scaffold the first NJI backend service from the HMCTS Crime SpringBoot template using `nji-scaffold.sh`,
...
**And** a Helm chart skeleton exists at `charts/nji-authorisation/` with `values-dev.yaml`, `values-staging.yaml`, `values-production.yaml` overlays (per AR24),
...
**And** a Postman collection skeleton exists at `postman/nji-authorisation-phase0.postman_collection.json` (per AR41).
...
**Given** this is the first service to scaffold in NJI,
**When** the engineer runs the `nji-architecture` Flyway baseline migration,
**Then** the shared `configuration_values` infrastructure table exists in the dev PostgreSQL instance,
**And** `nji-authorisation`'s DB role has `SELECT` on `configuration_values` (per FR8, AR19, AR22),
**And** `nji-authorisation`'s own service-owned Flyway migrations directory exists but is empty (tables created in Story 0.1.3).
...
**And** the configuration is documented in `nji-architecture/ADR-XXXX-data-at-rest-encryption.md`.

**Given** the Application Insights workspace is provisioned for NJI,
...
## Story 0.1.2: User can authenticate against `nji-mock-auth` and receive a JWT

As a **judicial user** (RSU, Court, Judge, Clerk, Finance, or MI/Reporting),
I want to authenticate against `nji-mock-auth` in non-prod environments using my email,
So that **NJI development and CI/UAT can proceed end-to-end without HMCTS IdP integration being live**, while preserving the same JWT shape that HMCTS IdP will issue at production cutover.
...
**Given** `nji-mock-auth` is starting up,
**When** the Spring profile in use is `production`,
**Then** the application refuses to start with a fatal error message *"nji-mock-auth must not be deployed to production"* (per AR35, gaps.md G5.3),
**And** the production `deploy-production.yml` workflow is configured to never deploy `nji-mock-auth`.

**Given** `nji-mock-auth` is seeded with a test user `judge.test@example.justice.gov.uk` mapped to a `Judge` role,
...
**Given** a service-token client `nji-payment-batch-client` is seeded in `nji-mock-auth`,
...
**Given** `nji-mock-auth` is deployed to dev AKS,
...
**Given** `nji-authorisation` is scaffolded per Story 0.1.1,
...
**And** the `nji_authorisation` DB role owns the tables (per AR19),
...
**And** validates the signature against the IdP's JWKS endpoint (`nji-mock-auth` in non-prod; HMCTS IdP from pre-Phase-9 cutover — configurable via Spring profile + Key Vault),
...
**Then** `uk.gov.hmcts.nji:api-nji-authorisation:1.0.0` is available,
...
I want to scaffold the `nji-ui` business-facing SPA repo with all NJI conventions (auth, design system, HTTP client, accessibility CI, Playwright),
...
**And** dependencies include GOV.UK Design System base + HMCTS/NJI extensions,
...
**Then** `HmctsIdpProvider.tsx` exposes the OIDC client context (configurable per environment via Vite env vars to point at `nji-mock-auth` in dev/CI and HMCTS IdP in pre-prod/prod),
...
**And** HMCTS / NJI design tokens (colours, spacing, typography) are applied,
...
**And** the deployment is independent of `nji-admin-ui` (per AR45b),
**And** the dev hostname (configurable in production to `nji.hmcts.gov.uk`) resolves to the new deployment.

**References:** FR55 (foundation only — Home content populated in Story 0.1.5), FR56; NFR17, NFR18, NFR19, NFR31, NFR40; AR42–AR45b.

---

## Story 0.1.5: User signs into NJI via SSO and lands on a role-scoped Home page

As a **judicial user** (RSU, Court, Judge, Clerk, Finance, MI/Reporting),
I want to sign into NJI via SSO, have my roles and Region/Area scope resolved, and see a Home page with navigation and tiles scoped to what I'm authorised to do,
So that **I can begin using NJI's workflows** — and at end of Phase 0 the platform pattern is demoable end-to-end across `nji-mock-auth` → `nji-authorisation` → `nji-ui` (per the Phase 0 demo gate).

**Acceptance Criteria:**

**Given** the user opens `nji-ui` while not authenticated,
**When** they navigate to any protected route,
**Then** `ProtectedRoute` redirects to the OIDC sign-in flow at `nji-mock-auth` (in non-prod),
**And** the user completes the dev-mode login (selects their seeded test user),
**And** they are redirected back to `nji-ui` with the authorisation code in the URL,
**And** the HTTP client exchanges the code for an ID token + access token,
**And** the user is redirected to `/home`.

**Given** the user is authenticated,
**When** the Home page renders,
**Then** the page shows a header with the NJI brand, user name, sign-out button, and a Region/Area selector populated with the regions/areas the user is authorised for (resolved via `useAuth().user.regions` from `nji-authorisation`'s `POST /authz/check`),
...
**Then** the OIDC end-session flow runs against `nji-mock-auth`,
**And** the user is redirected back to `nji-ui` and lands on an unauthenticated landing page,
**And** the access token and refresh token are cleared from client storage.

**Given** the user has an `auth_user_activation_flags` entry indicating their region is NOT activated for NJI yet,
**When** they land on Home,
**Then** they see a banner *"Your region has not yet migrated to NJI. Please continue using APEX."* and the workflow nav is disabled (per FR58 surface — full activation orchestration is Phase 9+).
...
**And** Postman collection `nji-authorisation-phase0.postman_collection.json` exercises `POST /v1/authz/check` and `GET /v1/users/{id}/effective-permissions` against the dev deployment.
