...
- **Story count:** 18 → **11**
- **Stories removed:**
  - Was 0.2.3 (Scaffold `nji-admin-ui`) — removed
  - Was 0.2.4 (Admin UI Reference Data module) — removed
  - Was 0.3.1 (`nji-authorisation` admin API extensions) — removed
...
- **Stories moved:**
  - Was 0.4.3 (OAuth `client_credentials` flow) — **moved to Phase 6** alongside `nji-payment-batch`
- **Stories renumbered:**
...
- **Partially covered (4) — data layer in Phase 0, UI surface deferred post-MVP:**
  ...
  - **FR56** — `nji-ui` business stack covered; `nji-admin-ui` admin stack moves post-MVP
...
- ✅ **Story sequencing within epics** — strictly sequential. The cross-epic dependency from Epic 0.4 onto `nji-admin-ui` is gone (admin UI removed).
- ✅ **File-churn check** — each epic targets its own slice. Cross-epic touches on `nji-authorisation` between Epic 0.1 (read API + JWT validation) and Epic 0.3 (ETL writes to the same auth tables) are additive (Epic 0.1 creates the schema; Epic 0.3 populates it).

## Architecture Compliance

- ✅ Starter template pattern (AR2–AR4) — Story 0.1.1 establishes; Stories 0.2.1, 0.4.1, and the mock-auth scaffold in 0.1.2 follow with canonical *"Scaffold NJI {service-name} from HMCTS starter"* commit.
