...
**Vertical slice:**
- `ram-reference-data` backend service scaffolded (per AR2–AR4)
...
- Reference Data **read-only** REST API: `GET` endpoints for consumption by `ram-ui`, downstream services, and OpenAPI clients. **No `POST`/`PUT`/`DELETE` endpoints** at Phase 0 — writes happen only via Flyway/SQL (the ETL load + future operational changes by DBA)
- Reference Data ETL stream at `ram-architecture/migration/reference-data/` (per AR46–AR48): reads APEX dumps, transforms to RAM Pathfinder shape, **loads via direct SQL INSERT statements** (not via API — API is read-only). The ETL is NOT a Flyway migration; it's a programme-level data-transform script per FR57 framing.
...
- `ram-admin-ui` Reference Data maintenance module with named-owner sign-off workflow
- Sign-off UI surface (sign-off in MVP is via git commits only)

---

## Story 0.2.1: Scaffold `ram-reference-data` service + 15 Reference Data tables via Flyway

As a **platform engineer**,
I want to scaffold `ram-reference-data` following the established pattern from Story 0.1.1 and create the 15 Reference Data tables via service-owned Flyway migrations,
So that **all other RAM Pathfinder services can read Reference Data via direct SQL on SELECT grants** (per FR7 revised, AR22) without each service having to own a copy of the schema.

**Acceptance Criteria:**

**Given** the engineer has manually pre-created the private GitHub repo `ram-reference-data` with branch protection on `main` via the GitHub web UI (per `ram-architecture/runbooks/github-setup.md`; the `gh` CLI is **not** available — see Story 0.1.1 for the canonical manual-setup pattern),
...
**And** `ram_reference_data` DB role owns the tables (per AR19),
...
**Then** every RAM Pathfinder service DB role (`ram_authorisation`, `ram_notification`, and placeholders for future domain service roles) has `SELECT` on all 15 Reference Data tables (per FR7 revised, AR22),
**And** the grants are tested by a CI integration test asserting each placeholder role can SELECT,
**And** no role other than `ram_reference_data` itself has INSERT/UPDATE/DELETE on these tables.
...
As an **API consumer** (`ram-ui` now; downstream services in Phase 1+),
...
**Given** `ram-reference-data` is scaffolded per Story 0.2.1,
...
**Then** `postman/ram-reference-data-phase0.postman_collection.json` exercises every read endpoint in the API,
...
**Given** the engineer creates the ETL at `ram-architecture/migration/reference-data/`,
**When** the ETL is implemented,
**Then** it reads APEX dumps (CSV files extracted from APEX, format defined per the migration spec at `ram-architecture/migration/reference-data/spec.md`),
**And** it transforms rows into RAM Pathfinder shapes per the mapping tables defined in `architecture/data-tables.md`,
**And** it **loads data via direct SQL INSERT statements** using the `ram_reference_data` DB role (the ETL runner is granted appropriate write access through a one-time DBA-issued credential held in Azure Key Vault),
**And** it is NOT a Flyway migration (RAM Pathfinder Flyway owns RAM Pathfinder DDL only; APEX-to-RAM Pathfinder data transform is a separate programme-level activity per FR57 framing),
...
**Then** the report at `ram-architecture/migration/reports/reference-data/{run-date}.md` shows:
   • source-row counts per APEX dump
   • target-row counts per RAM Pathfinder table
...
**And** updates are applied via `INSERT ... ON CONFLICT (natural_key) DO UPDATE SET ...` so deletions in APEX do NOT result in deletions in RAM Pathfinder (RAM Pathfinder is the system of record post-migration).

**Given** a named owner reviews the reconciliation report,
**When** they sign off via a versioned commit to `ram-architecture/migration/reports/reference-data/signoffs/{run-date}-{owner-handle}.md` (containing owner identity, timestamp, the specific report version signed off, and a short approval note),
...
**Given** the ETL encounters an anomaly it cannot resolve (e.g. an APEX value not in the RAM Pathfinder controlled list),
...
**And** the named owner can decide handling (correct in RAM Pathfinder controlled list / correct in APEX dump / drop) before re-run.
